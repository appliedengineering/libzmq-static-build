...
def download_and_extract_libsodium()
  ...
  pkg_name      = "libsodium-#{PKG_VER}-stable"
  pkg           = "#{pkg_name}.tar.gz"
  url           = "https://download.libsodium.org/libsodium/releases/#{pkg}"
  exit 1 unless system("cd #{BUILDDIR} && curl -O -L #{url}")
  exit 1 unless system("cd #{BUILDDIR} && tar xzf #{pkg}")
  FileUtils.mv "#{BUILDDIR}/libsodium-stable", "build/libsodium"
...
exit 1 unless system("cd #{BUILDDIR}/libsodium && ./autogen.sh")
...
for platform in PLATFORMS
  ...
  for arch in archs
    ...
    # exit 1 unless system("sed -i'.original' -e 's/#define HAVE_CLOCK_GETTIME 1/#undef HAVE_CLOCK_GETTIME/g' #{BUILDDIR}/libsodium/src/platform.hpp")
